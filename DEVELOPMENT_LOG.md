...
### 2026-09-24 节点和侧栏的视觉改版

- 用户需求：
  - 参数区更紧凑，把空间让给输入框；生成按钮缩小。
  - 节点更好看，参考了用户给的几张卡片设计图：
    - 外框包住画面；
    - 选中时像滑盖手机那样展开；
    - 名字放在吊牌上，拖动时会晃；
    - 选中时用白色的光，不用紫色边框。
  - 拖动节点不应该自动选中它；多选时节点不展开。
  - 左侧栏改成胶囊样式。
  - 深色模式下，卡片颜色要和背景协调。
- 改动：
  - `PromptComposer`：输入框下面是一行三个胶囊：模型、参数摘要（点开是浮层面板）、生成。只有一个模型时，模型胶囊不带箭头。上游便签的提示可以关掉。
  - 图片 / 视频节点：
    - 外框卡片，结构是：名字栏（`.node-topbar`）+ 画面 + 抽屉（`.node-drawer`）；
    - 新增 `NodeTag`（吊牌 + 拖动时的摆动计算）；
    - 展开和收起由 CSS 根据「单选、未在拖动」来控制，多选的判断用 `:has()`；
    - 深色和浅色模式各有一套外框配色。
  - `CanvasShell`：设置 `selectNodesOnDrag={false}`，拖动不再选中节点。
  - 侧栏改成胶囊加圆形按钮。
  - DESIGN.md 同步更新（颜色、尺寸、节点、侧栏）。
- 修复：
  - 圆角处外框底色漏边：收起时去掉卡片底色，画面自己描边。
  - 展开时卡片的阴影、光晕压到名字栏上，形成一道接缝：深色阴影去掉；光晕改由一层同时包住外框和名字栏的底层来画。逐像素确认过，接缝处颜色一致。
  - 抽屉内容撑宽卡片：给抽屉的网格列加了 `minmax(0, 1fr)`。
- 验证：
  - 前端 64 个测试通过，`tsc -b` 通过。
  - 在浏览器里逐一检查了：展开动画（抽屉高度 0 → 311px，约 0.3 秒）、拖动后不被选中、单击后展开、⌘ 多选时两个节点都不展开、吊牌摆动（拖动中约 ±18°，松手后衰减到 0）、深色和浅色两套配色。
- 未验证：Safari 上的 `:has()`、`backdrop-filter`、`grid-template-rows` 动画，需要在 Mac 上实际看一下。
